...
function fakeBackendFactory(backend, options, realBackend) {
    ...
    backend.connections.subscribe(function (connection) {
        ...
        setTimeout(function () {
            ...
            // update password
            if (connection.request.url.match(/\/api\/users\/\d+$/) && connection.request.method === http_1.RequestMethod.Get) {
                //getting user details in newPassword
                var newPassword = JSON.parse(connection.request.getBody());
                var urlParts = connection.request.url.split('/');
                var id = parseInt(urlParts[urlParts.length - 1]);
                for (var i = 0; i < users.length; i++) {
                    var user = users[i];
                    if (user.id === id) {
                        // delete old user password
                        user.password.splice(i, 1);
                        //add new password
                        user.password.splice(i, 0, "newPassword"); //user.newPassword //newPassword.password
                        localStorage.setItem('users', JSON.stringify(users));
                        break;
                    }
                }
            ...
            // reorder role
            if (connection.request.url.match(/\/api\/users\/\d+$/) && connection.request.method === http_1.RequestMethod.Post) {
                //getting user details in newRole
                var newRole = JSON.parse(connection.request.getBody());
                var newId = void 0;
                var newName = void 0;
                var urlParts = connection.request.url.split('/');
                var id = parseInt(urlParts[urlParts.length - 1]);
                ...
                for (var i = 0; i < users.length; i++) {
                    var user = users[i];
                    if (user.lastName === newRole) {
                        // storing old role
                        newName = user.lastName.splice(i);
                        newId = user.id;
                        //add new role
                        user.lastName.splice(i, 0, "newRole");
                        break;
                    }
                }
                for (var i = 0; i < users.length; i++) {
                    ...
                    if (newId.id === id) {
                        //reorder the role
                        newId.lastName.splice(i, 0, "newName");
                        break;
                    }
                }
                localStorage.setItem('users', JSON.stringify(users));
            ...
            /*  // reorder role
             if (connection.request.url.match('/api/users/') && connection.request.method === RequestMethod.Post) {
                 //getting user details in newRole
                 let newRole = JSON.parse(connection.request.getBody());
                 let newId;
                 let newName;
